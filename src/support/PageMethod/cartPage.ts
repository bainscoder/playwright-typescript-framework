...
export class CartPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.shoppingCartLink = page.getByRole("link", {name: /shopping cart/i,}).first();
    this.cartQuantity = page.locator(".cart-qty").first();
    this.quantityInput = page.locator(".qty-input");
    this.updateCartButton = page.getByRole("button", {name: "Update shopping cart",}).first();
    ...
    this.shippingAddressContinueButton = page.locator('[title="Continue"]').last();
    ...
    this.shippingAddressHeading = page.locator("h2").filter({hasText: "Shipping address",});
    this.shippingMethodHeading = page.locator("h2").filter({hasText: "Shipping method",});
    this.paymentMethodHeading = page.locator("h2").filter({hasText: "Payment method",});
    this.paymentInformationHeading = page.locator("h2").filter({hasText: "Payment information",});
    this.confirmOrderHeading = page.locator("h2").filter({hasText: "Confirm order",});
    ...
    this.billingAddressDropdown = page.locator("#billing-address-select").first();
...
const address = FakerUtils.generateBillingAddress();
  ...
  if (await this.billingAddressDropdown.isVisible()) {
    const selectedOption =
      await this.billingAddressDropdown
        .locator("option:checked")
        .textContent();
    if (
      selectedOption &&
      !selectedOption.includes("New Address")
    ) {
